<?php
	...
	if($act=='edit'){
		...
		$post['cat_more'] = short_check(get_args('cat_more'));
		$post['cat_more_cn'] = short_check(get_args('cat_more_cn'));
		$cat_id = intval(get_args('cat_id'));
		$post['sort'] = intval(get_args('sort'));
		...
		if(update_news_cat($dbo,$t_pro_cat,$post,$cat_id)) {
	...
	if($act=='add'){
		...
		$sql = "insert into `$t_pro_cat` $item_sql ";
		$dbo->exeUpdate($sql);
	...
	exit();
		...
		$sql="select * from $t_pro_cat where cat_id=".$r[3]."";
		$row=$dbo->getRow($sql);
?>