...
if($url_rewrite!='true'){
	...
	if(strpos($_SERVER['PHP_SELF'],"/index.php")){
		$_SERVER_PHP_SELF=str_replace('/index.php','/',$_SERVER['PHP_SELF']);
	}

	$baseUrl="http://".$HOSTURL;
	include	tpl_engine($SYSINFO['templates_name']."/module","page.html",1);
	...
	if($turn_page_url){
		...
		if($result['page']<=1){
			$result['preurl']=$baseUrl.$page_curt[0].'1'.$page_curt[1];
		}else{
			$result['preurl']=$baseUrl.$page_curt[0].($result['page']	-	1).$page_curt[1];
		}
		if($result['countpage']<=1){
			$result['nexturl']=$baseUrl.$page_curt[0].'1'.$page_curt[1];
		...
		if($last_page==1){
		$result['nopage']=$baseUrl.$page_curt[0].$pagecount.$page_curt[1];
		}else{
		$result['nopage']=$baseUrl.$page_curt[0].'1'.$page_curt[1];
		}
		//p($result);

		include	tpl_engine($SYSINFO['templates_name']."/module","page_rewritehtml.html",1);
		...
		include	tpl_engine($SYSINFO['templates_name']."/module","page_html.html",1);
	}
}
?>