	...
	<div class="container">
		<ul id="myTab" class="nav nav-tabs nav-stacked col-md-2 zhixunleft">
			...
			<?php foreach($other_list as $k => $v){?>
				<li <?php if($r[2]==$v['other_id']){?>class="active"<?php } ?> ><a href="<?php echo URL_NAV('about_'.$v['other_id']);?>" class="shoufei"><?php echo $v['title_cn'];?></a></li>
			<?php } ?>
			<?php } ?>
		</ul>
		<div id="myTabContent" class="tab-content col-md-9 zhixunright">
		    <div class="tab-pane fade in active">
				...
				<p class="zhixunp"><?php echo $other_info[title_cn];?></p>
				<p class="zhixunp2"><?php echo $other_info[content_cn];?></p>
		    </div>
		</div>
	</div>
</div><!-- /.blog-section -->
