...
<div class="inner-structure relative w100-l">
	<img src="images/zhixunbanner-en.jpg" alt="about-banner" />
...
<div class="blog-section w100-l fixed-padding">
	<div class="container">
		<ul id="myTab" class="nav nav-tabs nav-stacked col-md-2 zhixunleft">
			<?php if($cat_list){?>
				<li <?php if($r[2]==''){?>class="active"<?php } ?> ><a href="<?php echo URL_NAV('news');?>" class="shoufei">All information</a></li>
			<?php foreach($cat_list as $k => $v){?>
				<li <?php if($r[2]==$v['cat_id']){?>class="active"<?php } ?> ><a href="<?php echo URL_NAV('news_'.$v['cat_id']);?>" class="shoufei"><?php echo $v['cat_name'];?></a></li>
			<?php } ?>
			<?php } ?>
		</ul>
		<div id="myTabContent" class="tab-content col-md-9 zhixunright">
					...
					<?php foreach($result['result'] as $v){?>
						<div class="tab-pane fade in active">
							...
							<p class="zhixunp"><a href="<?php echo URL_NEWS('news',$v[cat_id],$v[article_id]);?>" target="_blank"><?php echo $v['title'];?></a></p>
							<p class="zhixunp2"><span class="post-date tcolor"><?php echo  $v[add_time];?></p>
						</div>
					<?php } ?>
					<?php }else{ ?>
					<div class='item' style='text-align:center;'>No content</div>
					<?php } ?>
					<div class="clear"></div>
		...
		<div class="blog-section w100-l fixed-padding">
			<div class="container">
				<div id="myTabContent" class="tab-content col-md-12 zhixunright">
								...
								<div class="tab-pane fade in active">
									<p class="zhixunp" style="text-align:center;"><?php echo $Article_Info[title];?></p>
									<p class="zhixunp2" style="text-align:center;"><small>column:<a href="<?php echo URL_NEWS('news',$Article_Info[cat_id]);?>"><?php echo  $show_cat_list[$Article_Info[cat_id]][cat_name];?></a> look:<?php echo  $Article_Info[click];?> Release time:<?php echo $Article_Info[add_time];?></small></p>
									...
									<p class="zhixunp3"><?php echo $Article_Info[content];?></p>
